...
class Coeff(LinearMixin):
    ...
    def __eq__(self, other):
        if isinstance(other, Coeff):
            return self.var == other.var and self.coeff == other.coeff
        else:
            return False
    def __ne__(self, other):
        return not (self == other)
    def __hash__(self):
        return hash((self.var, self.coeff))
...
class LinSys(object):
    ...
    def to_matrix(self, vars_indexes):
        ...
        for i, (vars, scalar) in enumerate(equations):      # @ReservedAssignment
            matrix[i, -1] = scalar
            for v, coeff in vars:
